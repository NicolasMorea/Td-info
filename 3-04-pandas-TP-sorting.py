...
df=pd.read_csv('titanic.csv',index_col='PassengerId').loc[:,['Survived','Pclass','Name','Sex','Age','Fare']]
print(df.columns.dtype)
plt.figure()
plt.plot(df.index,df.Age,'rs')
plt.figure()
plt.plot(df.Age,'b.')
#on observe le même graphique (sauf charte graphique, b pour bleu et . pour la forme en point)
#par défaut, plt utilise l'index de la dataframe pour les abscisses
...
print(df['Age'])
#sans affectation python, le tri n'est pas conservé
plt.figure()
df2=df.sort_values('Age')
plt.plot(np.linspace(1,len(df2),1),df2.Age,'b.') #???
...
print(df.loc[[673,746],'Age']) #ils ont le même âge
df2=df.sort_values(by=['Age','Fare'])

print(df2[df2['Age'] == 'NaN']) #???
print(df2['Age'].isna().value_counts()) # il manque 177 valeurs
#les valeurs manquantes sont à la fin
df.sort_values(by=['Age','Fare'],na_position='first')
...
# %%
df=pd.DataFrame(np.random.choice(100,4*5).reshape((4,5)),columns=['a', 'b', 'c','d','e'],index=['un','deux','trois','quatre'])
print(df)
print(df.sort_values('trois',axis=1))
...
# %%
df=pd.DataFrame(np.random.choice(100,4*5).reshape((4,5)),columns=['a', 'b', 'c','d','e'],index=['un','deux','trois','quatre'])
print(df.sort_values(df.index)) #??? je ne comprends pas la consigne
